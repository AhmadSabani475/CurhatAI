    ...
    try {
      ...
      if (storedMessages) {
        ...
        if (messages.length > 1) {
          ...
          setError('Tidak cukup percakapan untuk dianalisis.');
        }
      } else {
        setError('Tidak ada riwayat percakapan yang ditemukan.');
      }
    } catch (e) {
      setError('Gagal menganalisis percakapan.');
      console.error(e);
    } finally {
      setIsLoading(false);
    }
      ...
      {!analysis && !isLoading && !error && (
        <Card className="text-center p-10 border-2 border-dashed">
            <CardContent>
                ...
                <p className="text-muted-foreground mt-1">Klik tombol "Analisis Percakapan" untuk melihat topik dan ringkasan dari obrolanmu.</p>
            </CardContent>
        </Card>
      )}
